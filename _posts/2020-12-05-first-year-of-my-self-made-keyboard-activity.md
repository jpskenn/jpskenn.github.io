...
市販品のErgoDoxやKinesis Freestyleなどと比較しつつ、「肩にやさしい分離型で、左手でYを打鍵する癖（現在は矯正済み）に対応できる7列で、エルゴノミクスを感じる配列」という要望を満たす[ErgoDash](https://yushakobo.jp/shop/ergodash/)のキットを、初めての自作キーボードに選びました。

![初めての自作キーボード、ErgoDash](/assets/2020-12-05/IMG_0611.jpeg)

Mill-Maxソケットでソケット化する工夫をしたり、アンダーグローLEDを基板のおもて側につけるという定番のミスをやったりして組み立てました。  
...
「これなら基板設計なしでキーボードを作れる！」と、早速GitHubで公開されていた基板データをElecrowへ発注しましたが、一番安い発送方法のRegistered Mailにしたら到着まで1ヶ月ほど待たされてしまいました。
...
```
    ...
    - `matrix.c`は`quantum/matrix.c（※）`を改変して作成し、rules.mkと同じフォルダへ配置します。  
...
ほぼ知識ゼロからスタートした自キ活でしたが、「売ってないから、自分で作る」というとても単純なことを本気でやり、1年でとても具合の良いキーボードを作ることができました。  

[Colice](https://github.com/jpskenn/Colice)と[Jones](https://github.com/jpskenn/Jones)の製作では「これは無理かも」と思うようなときが何度もありましたが、”[サンボマスター / できっこないを やらなくちゃ](https://mora.jp/package/43000001/4988009064888/)”を聴いて気分を奮い立たせ、完成に至りました。  
また、これらのキーボードが作れたのは、先達の知恵のおかげです。  
特に[過去のアドベントカレンダー](https://scrapbox.io/self-made-kbds-ja/アドベントカレンダー)は知恵の宝庫で、読み返すたびに発見があり、何かしらの知識を与えてくれます。  

そんなアドベントカレンダーに、この大して情報量もない記事を寄稿するのはおこがましい限りですが、どこかの誰かに何か届けばいいなと思っています。

さて、今後の自キ活ですが、少しペースを落としつつ、Jones用オリジナルケースや分離型への展開、STM32化を進めていく予定です。  
また、僕にとってキーボードは何かをするための道具なので、いつまでもキーボードを作ってないで、何かができるようにしたいです。
